<section id="wrapper-footer" class="">
    <div class="dark-section">
        <div class="container text-white">
            <div class="row  text-center">
                <div class="col-lg-3 col-md-12 col-sm-12">
                    <h3 class="mb-4">Facebook</h3>
                    <div class="fb-page"
                         data-tabs="post"
                         style="width: "
                         data-href="https://www.facebook.com/toyntoys/"
                         data-width="380"></div>
                </div>
                <div class="col-lg-3 col-md-12 col-sm-12"><h3>Instagram</h3>
                    <div class="fb-page"
                         data-tabs="events"
                         data-href="https://www.instagram.com/toyntoysasia/"
                         data-width="380"></div>
                </div>

                <div class="col-lg-3 col-md-12 col-sm-12"><h3>Twitter</h3>
                    <a class="twitter-timeline" data-theme="dark"
                       href="https://twitter.com/Toyntoys1?ref_src=twsrc%5Etfw">Tweets by Toyntoys1</a>
                    <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>

                    <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
                </div>
                <div class="col-lg-3 col-md-12 col-sm-12 "><h6>Subscribe to our Newsletter</h6>
                    To follow
                </div>

            </div>
        </div>
    </div>
    <div class="gray-section">
        <div class="container text-white" style="">
            <p class="copyright-text">Copyright © 2020 ToyNToys. All rights reserved</p>

        </div>
    </div>


</section>


</body>
</html>
